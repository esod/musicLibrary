...
// This page is for deleting an album.
...
if (isset($_POST['submitted'])) {
	...
	if ($_POST['sure'] == 'Yes') { // Delete the record.
		...
		// Make the query:
		$q = "DELETE FROM esod_albums WHERE pk_albumid=$id LIMIT 1";		
		$r = @mysqli_query ($dbc, $q);
		if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
		
			// Print a message:
			echo '<p><strong>The album has been deleted.</strong></p>';	
		
		} else { // If the query did not run OK.
			echo '<p class="error">The album could not be deleted due to a system error.</p>'; // Public message.
			echo '<p>' . mysqli_error($dbc) . '<br />Query: ' . $q . '</p>'; // Debugging message.
		}
	
	} else { // No confirmation of deletion.
		echo '<p><strong>The album has NOT been deleted.</strong></p>';	
	}
	...
	<h3>Name: ' . $row[0] . '</h3>
	<p>Are you sure you want to delete this album?<br />
